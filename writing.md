...
This is clearfelt's editorial doctrine: what it considers bad writing, what it always preserves, and when it should not rewrite at all. It governs the product's judgment, not any one user's style, that layer is `voice.md` and `.clearfelt/voice-profile.md`. Read this before authoring a new rule, changing a scoring weight, or editing `prompts/audit_loop.xml`'s rewrite instructions: a change that contradicts this file is a signal to update this file first, not to quietly drift past it.
...
- **Writing quality is context-dependent.** The same sentence can be a tell in one register and correct in another. `rules/` flags surface patterns; `.clearfelt/domain.md`'s exemption list and `risk_tier` are how context overrides a surface match, not an afterthought bolted onto the score.
...
- **Domain-specific terms and target reading level** a project has explicitly declared exempt (`.clearfelt/domain.md`).
...
- **Any span wrapped in `<!-- clearfelt-lock -->` markers**, at every intensity tier, a narrower, per-span version of the same guarantee for content that doesn't need the whole project marked sensitive (`docs/decisions/0015`, `reference/rewrite.md`'s "Locked spans").
