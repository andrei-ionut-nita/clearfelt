...
One per line. The candidate must contain every one of these, or the run fails (see `check.hard_fail_on_constraint_violation` in `clearfelt.config.md`). Plain text matches as a literal, case-sensitive substring; wrap a line in `/pattern/flags` to match a regex instead.
