 ...
 *
 * Usage:
 *   node scripts/check.mjs --before <path> --after <path>
 *
...
const USAGE = `clearfelt preservation checker: diffs a source text against a rewrite candidate.

Usage:
  node scripts/check.mjs --before <path> --after <path>

Both paths must resolve inside the current project directory (process.cwd()).`;
...
function main() {
  ...
  const extractors = [
    ...
    ['date', extractDates],
    ['properNoun', extractProperNouns],
    ['quote', extractQuotes],
  ];
  const fingerprint = { dropped: [], added: [] };
  for (const [type, extractor] of extractors) {
    const result = buildFingerprint(null, extractor, type, beforeText, afterText);
  ...
  let verdict = 'pass';
  if (lockedSpanFailure || droppedFailure || addedFailure) verdict = 'fail';
  else if (lockedSpans.mismatches.length > 0 || fingerprint.dropped.length > 0 || fingerprint.added.length > 0) verdict = 'warn';
