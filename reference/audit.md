...
3. **If the target was a directory** (the JSON has `isDirectory: true` and a `files` array with more than one entry), present [reference/output-format.md](output-format.md)'s "Directory audit summary" template first: every filename named explicitly, a worst-first ranking table, and the greatest offender called out by name in the opening sentence. Never leave the reader to infer which or how many files were in scope. Follow it with each file's full breakdown, worst-first, each one still headed by its own filename.
4. Parse the JSON it prints and present it to the user following [reference/output-format.md](output-format.md)'s `/clearfelt audit` template: the filename, a plain-language verdict, and one-sentence reason first, `categoryPoints` as the "Why" table, `patternSummary` (with its `points` field, not raw severity) as the "Where" table, `breakdown.impacts` as "The full math" table, readability as its own table with plain-language bands, commentary (if any) as short bullets. Not a prose summary. All three data tables come pre-sorted by the script (points lost, descending): present them in that order, don't re-sort by category name or rule-file order, the whole point is that the reader sees the biggest driver first without having to scan for it. Always show the full-math table, not only when asked: two files can land on the same final score for different reasons (zero deductions left vs. adjustments cancelling out), and that table is what makes that legible instead of looking like a bug.
