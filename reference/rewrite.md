...
1. Resolve the target path. If `voice.mode` is `multi` (see `clearfelt-writing.config.md`) and no voice was named in the request, ask which voice applies before continuing, then pass it as `--voice <name>` to every `scripts/detect.mjs` call below.
...
3. Read `human_score_threshold` and `max_iterations` from `clearfelt-writing.config.md` before starting. Intensity is resolved separately, see below, not read silently.
...
- The score at each tier is computed from the actual resulting text, the same way every other clearfelt-writing score is, never assigned by which tier ran. Scope escalation usually produces real score increases tier over tier, since each tier removes strictly more of what the score deducts for, but that's an earned consequence of what changed, not a guaranteed number.
- If you've seen a "no-touch / minimal-touch / transform / aggressive" naming elsewhere (an earlier v0.3 planning document used it), it's the same ladder under different names, not a second system. `light_touch` and `balanced` are the minimal-touch end, `full_rewrite` is transform, `structural_rework` is the aggressive end. "No-touch" already exists too, as declining to run `/clearfelt-writing rewrite` at all, or as `risk_tier: sensitive`'s hedge/qualifier protection for the spans it covers, not a fifth tier.

**Resolving which tier runs**, checked in Pass 1 before any rewriting happens:

1. Check `.clearfelt-writing/domain.md`'s `preferred_intensity` field. If set, use it (project-level, shared by everyone on the project).
2. Otherwise check `~/.clearfelt-writing/settings.md`'s `intensity` and `rewrite.ask_intensity`. If set, use it (a user's own global save, see "Saving a preference" below).
3. Otherwise fall back to `clearfelt-writing.config.md`'s shipped default, `rewrite.ask_intensity: true`, which means: ask.
...
- **Global** ("every project using this skill install"): writes `intensity` and `rewrite.ask_intensity: false` into `~/.clearfelt-writing/settings.md`, in the user's home directory, creating the file and its `## Rewrite` section if needed. This file is never shipped with the skill and never touched by a `git pull` or reinstall of the skill itself, so a global save survives updates. Do not write a saved preference into `clearfelt-writing.config.md`: that file lives inside the skill's own repo and gets reset by an update.
- **This project only**: writes `preferred_intensity` into `.clearfelt-writing/domain.md` (creating it from `templates/domain.example.md` if it doesn't exist yet). This already lives in the user's project directory, not the skill's repo, so it's safe from updates by construction, same as `.clearfelt-writing/voice-profile.md` always has been.

## Locked spans

Wrap any span in a target file with `<!-- clearfelt-writing-lock -->` and `<!-- /clearfelt-writing-lock -->` on their own lines to keep it exactly as-is through every intensity tier, including `structural_rework`. Narrower than `risk_tier: sensitive`: that protects two whole rule categories (`frictionless_claims`, `weasel_attribution`) project-wide; a locked span protects whatever text you actually wrapped, nothing more, nothing less, and works without declaring the whole project sensitive. Use it for a verbatim quote, a legal boilerplate footer, a pull-quote that has to stay exact, anything where the reason to preserve it is "this specific text," not "this whole document."

A rule hit inside a locked span still counts in `/clearfelt-writing audit`'s score and report exactly as it would anywhere else, locking a span changes what `/clearfelt-writing rewrite` is willing to touch, not what `scripts/detect.mjs` reports or scores. The markers themselves are ordinary HTML comments, invisible in rendered Markdown, and are left in place by `/clearfelt-writing rewrite`, not stripped, so the same region stays protected on the next run too.
...
- **Inline, zero-setup**: `--max-chars <n>`, `--max-words <n>`, `--must-contain <text-or-/regex/>`, `--must-not-contain <text-or-/regex/>` on the `/clearfelt-writing rewrite` invocation. `--must-contain`/`--must-not-contain` can repeat.
- **Named, reusable**: `.clearfelt-writing/constraints/<name>.md` (copy `templates/constraints.example.md`), referenced with `--constraints <name>`. Inline flags layer on top rather than replacing a named set: `--max-chars`/`--max-words` override the file's own value, `--must-contain`/`--must-not-contain` add to its lists.

Every scrub pass in the loop below should target an active constraint directly, aiming the rewrite at the limit rather than just hoping it lands under it, the same way it already respects a locked span mid-loop instead of only checking afterward. This is verified for real, not just by instruction: the same `scripts/check.mjs` call the "Preservation checking" section below describes checks it. Exceeding `max_chars`/`max_words` always blocks the write. A `must_contain`/`must_not_contain` miss blocks by default too, `check.hard_fail_on_constraint_violation` in `clearfelt-writing.config.md`, on by default unlike the fact-preservation toggles below, since a constraint is something the user explicitly declared, not a heuristic guess.

## Preservation checking

The no-fabrication rule, locked-span guarantee, and any active constraints are not enforced by prompt instruction alone. Pass 3 of `prompts/audit_loop.xml` writes the current candidate to `.clearfelt-writing/candidate.md` and runs `node scripts/check.mjs --before <path> --after .clearfelt-writing/candidate.md [constraint flags]`, which deterministically diffs the source against the candidate:

- Every `<!-- clearfelt-writing-lock -->` span is checked for byte-identical content. A mismatch always blocks the write, see `check.hard_fail_on_locked_span_mismatch` in `clearfelt-writing.config.md`.
...
`.clearfelt-writing/domain.md` can set `risk_tier: sensitive` for a project where a rewrite carries real legal or reputational weight (a shareholder letter, a regulatory filing, anything with safe-harbor language, anything already reviewed by Legal). When set:

- Hedges, qualifiers, and attributions (the `frictionless_claims` and `weasel_attribution` rule categories) are never rewritten away, at any intensity tier, including `structural_rework`. That language is often legally load-bearing in this kind of document, not a stylistic weakness clearfelt-writing should be removing.
- The confirm-before-write gate below is forced on for this project, regardless of `rewrite.require_confirmation` in `clearfelt-writing.config.md` or any saved global preference in `~/.clearfelt-writing/settings.md`.
...
`rewrite.require_confirmation` in `clearfelt-writing.config.md` defaults to `true`. This is the safe default and should not be treated as optional in practice, and is a separate, later checkpoint from the intensity question above, not a replacement for it:
...
2b. Once the loop ends, run `node scripts/detect.mjs --mode report .clearfelt-writing/candidate.md [--voice <name>]` once (not per iteration; `--mode score` is what the loop itself uses, and deliberately never carries this field, see docs/decisions/0024) to check for a `registerCheck`. If present, mention it as its own bullet under "What changed, and why," never folded into the score explanation: it's a tone-match note, not something the rewrite passed or failed on.
3. Ask through the harness's own structured single-choice question tool (Claude Code: `AskUserQuestion`), never a bare yes/no prompt, same wizard rule `reference/setup.md` uses: **Overwrite `<path>`** / **Write to a new file** / **Other**. Don't proceed on an ambiguous or ignored response.
   ...
   - **Write to a new file**: default suggested target is `<stem>.rewritten<ext>` next to the original (mirrors `/clearfelt-writing write`'s `<seed-stem>.draft.md` convention), shown as part of the option's description so the user sees it before picking. If the user wants a different name, they type it into the picker's own "Other" slot rather than being asked a second question; resolve whatever they typed as the write target directly.
...
This mirrors clearfelt-writing's own no-fabrication and no-silent-changes posture: the tool never modifies a user's file, or removes anything, without the user having seen what's about to change and said yes.
...
**Voice-profile precedence.** If `.clearfelt-writing/voice-profile.md` exists in the user's project and states a preference (for example, "I like using em-dashes" or a word the base rules would otherwise flag), that preference overrides the shipped rule files for this project. `scripts/detect.mjs` already checks the voice profile before flagging a hit, so a rewritten draft should never fight the user's own stated voice.

## Voice profile

If `.clearfelt-writing/voice-profile.md` doesn't exist yet, mention once, before starting, that `/clearfelt-writing setup` can build a personal one first. If the user would rather proceed anyway, continue with bundled defaults from `templates/voice-profile.example.md`. Don't block the rewrite run on it.
