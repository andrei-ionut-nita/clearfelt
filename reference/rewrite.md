...
1. Present a before/after following [reference/output-format.md](output-format.md)'s `/clearfelt rewrite` template: `## Before` and `## After` as separate headed sections (not run into one line), each with its score, a "Changes made" bullet list, not a prose paragraph explaining the diff.
