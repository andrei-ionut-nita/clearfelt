...
`/clearfelt rewrite` and `/clearfelt audit` already surface *some* resolved state inline (which intensity is running and why, whether a project is marked sensitive), but only the pieces relevant to that one run, and only once, in the middle of another command's output. `/clearfelt explain` is the one place to see everything at once, before committing to a run: which voice profile is active and how many words it overrides, whether a domain profile exists and what its risk tier and preferred intensity are, every config setting's final value and which of the three layers (default, shipped `clearfelt.config.md`, global `~/.clearfelt/settings.md`) it actually came from, and whether the auto-audit hook is currently on.
...
- `config`: every setting from `clearfelt.config.md`, each as `{ value, source }` where `source` is `"default"`, `"shipped (clearfelt.config.md)"`, or `"global (~/.clearfelt/settings.md)"`.
- `hook`: `enabled`, `quiet`, `ignoreRules`, `ignoreFiles` (same shape `$clearfelt hooks status` prints).
